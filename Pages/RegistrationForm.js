...
const Bubble = ({label, selected, onPress, type}) => {
  ...
  const backgroundColor = selected
    ? type === 'like' ? '#4CAF50' : '#E53935' // green or red
    : '#CCC';
...
const RegistrationForm = () => {
  ...
  { id: Date.now(), name: '', phone: '', email: '', isNew: true },
]);
...
const removeMedication = index => {
  ...
  const validateAccountInfo = () => {
    ...
    if (!dailyCalories.trim()) newErrors.dailyCalories = 'Daily Calories is required.';

    if (likedMeals.length < 3) newErrors.likedMeals = 'Select at least 3 liked meals.';
    if (dislikedMeals.length < 3) newErrors.dislikedMeals = 'Select at least 3 disliked meals.';
    ...
    {accountFields.map(({label, value, setter}) => (
      <React.Fragment key={label}>
        ...
        <TextInput
          ...
          style={[
            styles.input,
            focusedField === label && { borderColor: '#007bff', borderWidth: 2 },
          ]}
          value={value}
  ...
  <Text style={{ color: 'red', marginLeft: 5, marginTop: 5 }}>
   ⚠️  {errors.dislikedMeals}
      ...
      <TextInput
        style={[
          styles.input,
          focusedField === `medName${index}` && { borderColor: '#007bff', borderWidth: 2 },
        ]}
  ...
  <View style={styles.timePickerContainer}>
    ...
    <View style={styles.frequencyRow}>
      {['daily', 'weekly', 'monthly'].map((freq) => (
        <TouchableOpacity
          ...
          style={[
            styles.freqButton,
            medicationTime[index]?.freq === freq && styles.freqButtonSelected,
          ]}
          onPress={() =>
            setMedicationTime((prev) => ({
              ...prev,
        ...
        >
          <Text style={{ color: medicationTime[index]?.freq === freq ? '#fff' : '#000' }}>
            {freq}
          </Text>
        </TouchableOpacity>
      ))}
    ...
    <TouchableOpacity
      ...
      onPress={() => {
        ...
        const validHour = t?.hour && /^\d{2}$/.test(t.hour) && parseInt(t.hour) >= 0 && parseInt(t.hour) <= 23;
        const validMinute = t?.minute && /^\d{2}$/.test(t.minute) && parseInt(t.minute) >= 0 && parseInt(t.minute) <= 59;
        ...
        if (validHour && validMinute && t?.freq) {
          ...
          alert('Please enter a valid time (00-23 for hour and 00-59 for minute) and select frequency.');
        }
      }}
    >
      ...
      {medicalConditions.map((condition, index) => (
        <React.Fragment key={index}>
          <TextInput
            style={[
              styles.input,
              focusedField === `condition${index}` && { borderColor: '#007bff', borderWidth: 2 },
            ]}
      ...
      {dietaryRestrictions.map((restriction, index) => (
        <React.Fragment key={index}>
            <TextInput
              style={[
                styles.input,
                focusedField === `restriction${index}` && { borderColor: '#007bff', borderWidth: 2 },
              ]}
